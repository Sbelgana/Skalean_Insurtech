...
**Fin du document task-4.3.1-app-skeleton-layouts-middleware-auth-i18n.md.**

**Reference meta-prompt** : `00-pilotage/meta-prompts/B-16-sprint-16-web-broker-app.md` Tache 4.3.1.
**Decisions strategiques** : decision-002, decision-005, decision-006, decision-008, decision-009, decision-010.
**Sprint** : 16 / 35 -- Phase 4 Vertical Insure -- web-broker bootstrap functionnel.
**Effort confirme** : 6h (4h code + 1h tests + 0.5h validation + 0.5h PR/review).
**Pre-conditions** : Sprint 4 livre, Sprint 5 livre, Sprint 6 livre, Sprint 7 livre.
**Post-conditions** : Sprint 16 tache 4.3.2 (Pages Auth) peut demarrer.
...
const PROTECTED_ROUTES = [
  '/dashboard', '/contacts', '/companies', '/deals', '/polices',
  '/broker-queue', '/sinistres', '/parametres', '/profile',
] as const;

const AUTH_ROUTES = [
  '/login', '/verify-mfa', '/signup', '/forgot-password',
  '/reset-password', '/verify-email', '/select-tenant', '/email-sent',
] as const;
...
function isJwtValid(token: string | undefined): SkaleanJwtPayload | null {
  ...
  try {
    ...
    if (!payload?.exp) return null;
    const nowSec = Math.floor(Date.now() / 1000);
    if (payload.exp - JWT_EXPIRY_MARGIN_SEC < nowSec) return null;
    return payload;
  } catch {
    return null;
  }
...
export default async function middleware(request: NextRequest): Promise<NextResponse> {
  ...
  if (PUBLIC_ROUTES.includes(cleanPath as (typeof PUBLIC_ROUTES)[number])) return intlResponse;
    ...
    >
      <body className="min-h-screen bg-background text-foreground antialiased font-sans">
        <ThemeProvider attribute="class" defaultTheme="system" enableSystem disableTransitionOnChange>
          ...
          <Toaster
            position={RTL_LOCALES.has(locale) ? 'top-left' : 'top-right'}
            richColors closeButton dir={dir} theme="system"
          />
        </ThemeProvider>
      </body>
    </html>
...
export default async function AuthLayout({ children, params }: AuthLayoutProps) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-muted/30">
      <header className="flex items-center justify-between px-6 py-4 sm:px-12">
        <a href={`/${locale}`} className="flex items-center gap-3" aria-label={t('aria.back_home')}>
          <Image src="/icons/skalean-logo.svg" alt="Skalean Broker"
            width={140} height={32} priority className="h-8 w-auto" />
        </a>
        <div className="flex items-center gap-3">
          <LocaleSwitcher />
          <ThemeToggle />
...
```typescript
/**
 * Protected route group layout -- pages authentifiees
 * Reference : 00-pilotage/meta-prompts/B-16-sprint-16-web-broker-app.md (Tache 4.3.1)
 */
import { redirect } from 'next/navigation';
import { headers } from 'next/headers';
...
import { apiServerGet } from '@/lib/api-client.server';
import type { AppLocale } from '@/lib/i18n/config';
import type { TenantSummary } from '@/types/session';
...
export default async function ProtectedLayout({ children, params }: ProtectedLayoutProps) {
  ...
  await Promise.all([
    ...
    queryClient.prefetchQuery({
      ...
      queryFn: async () => {
        try {
          return await apiServerGet<TenantSummary>('/api/v1/tenants/me');
        } catch {
          return null;
        }
      },
          ...
          <div className="flex min-h-screen w-full" data-broker-layout="protected">
            ...
            <div className="flex flex-1 flex-col">
              <header
                ...
                data-placeholder="topbar"
              >
                <p className="text-sm text-muted-foreground">{t('placeholder.topbar_note')}</p>
              </header>
              <main className="flex-1 overflow-y-auto px-4 py-6 sm:px-6 lg:px-8">
                {children}
              </main>
...
function SentryErrorBoundary({ children }: { children: React.ReactNode }) {
  return (
    <Sentry.ErrorBoundary
      fallback={({ error, resetError }) => (
        <div role="alert" className="m-8 rounded-lg border border-destructive/40 bg-destructive/5 p-6 text-destructive">
          <h2 className="text-lg font-semibold">Une erreur est survenue</h2>
          <pre className="mt-2 max-h-40 overflow-auto text-xs">
            {(error as Error)?.message ?? 'Unknown error'}
          </pre>
...
export function Providers({ children, locale, initialSession, initialTenantId, dehydratedState }: ProvidersProps) {
  const queryClient = useState(() => getQueryClient())[0];
  const sentryInitialized = useRef(false);
 ...
 *
 * Roles :
 *   - Auto-injection Authorization Bearer (cookie SSR / zustand client)
 *   - Auto-injection x-tenant-id depuis tenant-store ou cookies SSR
 *   - Auto-injection x-trace-id via crypto.randomUUID
 *   - Auto-injection Idempotency-Key sur POST/PUT/PATCH/DELETE (jamais GET)
 *   - Auto-injection Accept-Language depuis next-intl locale
 *   - 401 -> trigger refresh token via /api/auth/refresh (singleton promise dedupe)
 *   - 5xx -> Sentry.captureException + logger error
 */
import axios, {
...
async function refreshAccessToken(): Promise<boolean> {
  ...
  refreshPromise = (async () => {
    try {
      ...
      if (!response.ok) {
        logger.warn({ component: 'api-client', status: response.status }, 'refresh_token_failed');
        return false;
      }
      logger.debug({ component: 'api-client' }, 'refresh_token_success');
      return true;
    } catch (err) {
      logger.error({ component: 'api-client', err: (err as Error).message }, 'refresh_token_network_error');
      return false;
    } finally {
...
export function createApiClient(options: ApiClientOptions = {}): AxiosInstance {
  ...
  instance.interceptors.request.use(
    (config: InternalAxiosRequestConfig) => {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (tenantId) {
          ...
          logger.warn({ component: 'api-client', url: config.url, method }, 'tenant_id_missing_warning');
        }
      }
      ...
      const locale = options.locale ?? (typeof document !== 'undefined' ? document.documentElement.lang : 'fr');
      config.headers.set('Accept-Language', locale);
    ...
    (error: AxiosError) => {
      logger.error({ component: 'api-client', err: error.message }, 'request_interceptor_error');
      return Promise.reject(error);
    }
  );
  ...
  instance.interceptors.response.use(
    ...
    async (error: AxiosError<ApiErrorBody>) => {
      ...
      if (RETRY_STATUS_CODES.has(status) && originalConfig && !originalConfig._retry) {
        originalConfig._retry = true;
        logger.warn({ component: 'api-client', status, url: originalConfig.url }, 'retry_5xx');
        await new Promise((resolve) => setTimeout(resolve, 500));
        return instance.request(originalConfig);
      }
      ...
      if (status >= 500) {
        Sentry.captureException(error, {
          tags: { component: 'api-client', status: String(status) },
          extra: { url: originalConfig?.url, method: originalConfig?.method },
        });
        logger.error({ component: 'api-client', status, url: originalConfig?.url, method: originalConfig?.method }, 'server_error_5xx');
      }

      if (status === 403) {
        logger.warn({ component: 'api-client', url: originalConfig?.url }, 'forbidden_403');
      }
...
export async function apiServerFetch<T>(path: string, options: ServerFetchOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    logger.error({ component: 'api-client.server', status: response.status, path }, 'server_fetch_failed');
    throw apiErr;
  }
  return (await response.json()) as T;
}
...
export function decodeJwtUnsafe(token: string | null | undefined): SkaleanJwtPayload | null {
  ...
  try {
    ...
    logger.warn({ component: 'jwt', err: (err as Error).message }, 'jwt_decode_failed');
    return null;
  }
}

export function isJwtExpired(payload: SkaleanJwtPayload | null, marginSeconds: number = EXPIRY_MARGIN_SEC): boolean {
  if (!payload) return true;
  const nowSec = Math.floor(Date.now() / 1000);
  return payload.exp - marginSeconds < nowSec;
}
...
export function jwtHasRole(payload: SkaleanJwtPayload | null, roles: string | string[]): boolean {
  if (!payload?.role) return false;
  const list = Array.isArray(roles) ? roles : [roles];
  return list.includes(payload.role);
}
...
export async function requireRole(roles: string | string[]): Promise<UserSession> {
  ...
  const list = Array.isArray(roles) ? roles : [roles];
  if (!list.includes(session.role)) {
    logger.warn({ component: 'auth.session', userRole: session.role, requiredRoles: list }, 'require_role_forbidden');
    redirect('/fr/dashboard?error=forbidden');
  }
  return session;
}
...
export async function requirePermission(permission: string): Promise<UserSession> {
  const session = await requireSession();
  if (!session.permissions.includes(permission)) {
    logger.warn({ component: 'auth.session', userId: session.userId, permission }, 'require_permission_forbidden');
    redirect('/fr/dashboard?error=forbidden');
  }
  return session;
}
...
export async function setAccessTokenCookie(token: string, options: SetAuthCookieOptions = {}): Promise<void> {
  const cookieStore = await cookies();
  cookieStore.set('access_token', token, {
    httpOnly: true,
    secure: isProd,
    sameSite: 'lax',
    path: '/',
    maxAge: options.maxAge ?? 900,
  });
}

export async function setRefreshTokenCookie(token: string, options: SetAuthCookieOptions = {}): Promise<void> {
  const cookieStore = await cookies();
  cookieStore.set('refresh_token', token, {
    httpOnly: true,
    secure: isProd,
    sameSite: 'strict',
    path: '/api/auth',
    maxAge: options.maxAge ?? 2_592_000,
  });
}
...
export const I18N_NAMESPACES = [
  'meta', 'home', 'common', 'nav', 'auth', 'tenant', 'errors',
  'middleware', 'protected', 'dashboard', 'contacts', 'companies',
  'deals', 'polices', 'broker_queue', 'sinistres', 'parametres', 'profile',
] as const;
```
...
export default getRequestConfig(async ({ requestLocale }) => {
  let locale = await requestLocale;
  if (!locale || !isSupportedLocale(locale)) locale = DEFAULT_LOCALE;

  let messages: Record<string, unknown>;
  try {
    messages = (await import(`@/messages/${locale}.json`)).default;
  } catch (error) {
    logger.error({ component: 'i18n.request', locale, err: (error as Error).message }, 'messages_load_failed_fallback_fr');
    if (process.env.NODE_ENV !== 'production') {
  ...
  return {
    ...
    formats: {
      dateTime: {
        ...
        long: { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit', timeZoneName: 'short' },
      },
      number: {
        currency: { style: 'currency', currency: 'MAD', maximumFractionDigits: 2 },
        percent: { style: 'percent', maximumFractionDigits: 1 },
    ...
    onError(error) {
      if (error.code === 'MISSING_MESSAGE') {
        logger.warn({ component: 'i18n.request', locale, code: error.code, message: error.message }, 'missing_message_key');
        return;
      }
      logger.error({ component: 'i18n.request', locale, code: error.code, message: error.message }, 'i18n_runtime_error');
    },
    getMessageFallback({ namespace, key, error }) {
      const fullKey = namespace ? `${namespace}.${key}` : key;
      if (error.code === 'MISSING_MESSAGE') {
...
const TIMEZONE = 'Africa/Casablanca';
const DATE_FNS_LOCALE_MAP = { fr, 'ar-MA': arMA, ar } as const;
...
export function formatDate(date: Date | string | number, locale: AppLocale, formatString = 'dd MMM yyyy'): string {
  return formatInTimeZone(date, TIMEZONE, formatString, { locale: getDateFnsLocale(locale) });
}

export function formatDateTime(date: Date | string | number, locale: AppLocale, formatString = 'dd MMM yyyy HH:mm'): string {
  return formatInTimeZone(date, TIMEZONE, formatString, { locale: getDateFnsLocale(locale) });
}

export function formatRelative(date: Date | string | number, baseDate: Date, locale: AppLocale): string {
  const zonedDate = toZonedTime(typeof date === 'string' || typeof date === 'number' ? new Date(date) : date, TIMEZONE);
  const zonedBase = toZonedTime(baseDate, TIMEZONE);
  return formatRelativeDateFns(zonedDate, zonedBase, { locale: getDateFnsLocale(locale) });
}
...
export const useTenantStore = create<TenantState>()(
  persist(
    ...
    }),
    {
      ...
      partialize: (state) => ({ tenantId: state.tenantId, tenantName: state.tenantName }),
    }
  )
);
```
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        logger.warn({ component: 'SessionProvider', err: (err as Error).message }, 'auth_me_refresh_failed');
        throw err;
      }
    },
    enabled: Boolean(initialSession),
...
export function TenantContextSync({ children, initialTenantId }: TenantContextSyncProps) {
  ...
  useEffect(() => {
    if (!hasHydrated) return;
    if (tenantId && lastSyncedRef.current && tenantId !== lastSyncedRef.current) {
      logger.info({ component: 'TenantContextSync', from: lastSyncedRef.current, to: tenantId }, 'tenant_switched_refreshing_rsc');
      lastSyncedRef.current = tenantId;
      router.refresh();
    }
  }, [tenantId, hasHydrated, router]);
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!refreshToken) {
    logger.warn({ component: 'api.auth.refresh' }, 'refresh_token_cookie_missing');
    return NextResponse.json({ code: 'NO_REFRESH_TOKEN', message: 'Refresh token absent' }, { status: 401 });
  }
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn({ component: 'api.auth.refresh', status: response.status }, 'backend_refresh_failed');
      await clearAuthCookies();
      return NextResponse.json({ code: 'REFRESH_FAILED', message: 'Refresh failed' }, { status: 401 });
    }
    ...
    logger.info({ component: 'api.auth.refresh' }, 'refresh_success');
    return NextResponse.json({ success: true, expires_in: data.expires_in }, { status: 200 });
  } catch (err) {
    logger.error({ component: 'api.auth.refresh', err: (err as Error).message }, 'refresh_unexpected_error');
    await clearAuthCookies();
    return NextResponse.json({ code: 'INTERNAL_ERROR', message: 'Internal error during refresh' }, { status: 500 });
  }
}
```
...
export async function POST(_request: Request): Promise<NextResponse> {
  ...
  if (accessToken) {
    try {
      ...
      logger.warn({ component: 'api.auth.signout', err: (err as Error).message }, 'backend_signout_failed_continue');
    }
  }
...
export async function GET(_request: Request): Promise<NextResponse<UserSession | { error: string }>> {
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn({ component: 'api.auth.me', status: response.status }, 'backend_me_failed');
      return NextResponse.json({ error: 'Backend error' }, { status: response.status });
    }
    const data = await response.json();
    const session: UserSession = {
    ...
    logger.error({ component: 'api.auth.me', err: (err as Error).message }, 'me_unexpected_error');
    return NextResponse.json({ error: 'Internal error' }, { status: 500 });
  }
}
```
...
interface BackendTenantMembership { id: string; name: string; }
interface BackendTenantsMeResponse { memberships: BackendTenantMembership[]; }
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    logger.warn({ component: 'api.auth.switch-tenant', err: (err as Error).message }, 'invalid_body');
    return NextResponse.json({ error: 'Invalid body' }, { status: 400 });
  }
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn({ component: 'api.auth.switch-tenant', status: response.status }, 'tenants_me_failed');
      return NextResponse.json({ error: 'Cannot verify tenant access' }, { status: 502 });
    }
    const data: BackendTenantsMeResponse = await response.json();
    const allowed = data.memberships.some((m) => m.id === parsed.tenant_id);
    if (!allowed) {
      logger.warn({ component: 'api.auth.switch-tenant', userId: payload.sub, tenantId: parsed.tenant_id }, 'tenant_access_denied');
      return NextResponse.json({ error: 'Access denied to this tenant' }, { status: 403 });
    }

    await setTenantIdCookie(parsed.tenant_id);
    logger.info({ component: 'api.auth.switch-tenant', userId: payload.sub, tenantId: parsed.tenant_id }, 'tenant_switched');
    return NextResponse.json({ success: true, tenant_id: parsed.tenant_id }, { status: 200 });
  } catch (err) {
    logger.error({ component: 'api.auth.switch-tenant', err: (err as Error).message }, 'switch_tenant_unexpected_error');
    return NextResponse.json({ error: 'Internal error' }, { status: 500 });
  }
}
```
...
### 6.24 `repo/apps/web-broker/src/messages/fr.json` (~120 lignes)

```json
{
  "meta": {
    "title": { "default": "Skalean Broker -- Espace courtier", "brand": "Skalean Broker" },
    "description": "Plateforme de gestion courtage en assurance Skalean InsurTech : contacts, polices, sinistres, commissions."
  },
  "home": {
    "title": "Bienvenue sur Skalean Broker",
    ...
    "placeholder": { "banner": "Cette page sera remplacee par le tableau de bord courtier dans le Sprint 17." }
  },
  "common": {
    ...
    "next": "Suivant", "previous": "Precedent", "search": "Rechercher", "filter": "Filtrer",
    "sort": "Trier", "refresh": "Rafraichir", "export": "Exporter"
  },
  "nav": {
    "dashboard": "Tableau de bord", "contacts": "Contacts", "companies": "Entreprises",
    "deals": "Affaires", "policies": "Polices", "claims": "Sinistres",
    "broker_queue": "File de validation", "settings": "Parametres", "profile": "Profil"
  },
  "auth": {
    "login": "Connexion", "logout": "Deconnexion", "signin": "Se connecter",
    "signup": "Creer un compte", "forgotPassword": "Mot de passe oublie",
    "resetPassword": "Reinitialiser le mot de passe", "verifyEmail": "Verifier l'email",
    "verifyMfa": "Verifier le code MFA", "selectTenant": "Selectionner le cabinet",
    "aria": { "back_home": "Retour a l'accueil" },
    "footer": {
      "copyright": "Copyright {year} Skalean InsurTech. Tous droits reserves.",
      "privacy": "Politique de confidentialite", "cookies": "Cookies", "cndp_notice": "Notice CNDP"
    }
  },
  "tenant": {
    "switcher": { "label": "Cabinet courtier", "placeholder": "Choisir un cabinet" },
    "required": "Selection d'un cabinet requise", "access_denied": "Acces refuse a ce cabinet"
  },
  "errors": {
    "network": "Connexion reseau impossible", "unauthorized": "Authentification requise",
    "forbidden": "Acces refuse", "notFound": "Ressource introuvable",
    "serverError": "Erreur serveur", "validation": "Donnees invalides",
    "session_expired": "Votre session a expire. Veuillez vous reconnecter.",
    "refresh_failed": "Impossible de prolonger la session",
    "tenant_required": "Veuillez selectionner un cabinet courtier pour continuer"
  },
  ...
  "protected": {
    ...
    "placeholder": {
      "sidebar_note": "Navigation principale livree dans le Sprint 16 tache 4.3.3.",
      "topbar_note": "Barre superieure livree dans le Sprint 16 tache 4.3.3."
    }
  }
}
```

### 6.25 `repo/apps/web-broker/src/messages/ar-MA.json` (~120 lignes Darija)

```json
{
  "meta": {
    "title": { "default": "Skalean Broker", "brand": "Skalean Broker" },
    "description": "منصة Skalean InsurTech ديال السماسرة ديال التامين."
  },
  "home": {
    "title": "مرحبا بيك فـ Skalean Broker",
    "palette": {
    ...
    "placeholder": { "banner": "هاد الصفحة غادي تتبدل بـ Dashboard فـ Sprint 17." }
  },
  "common": {
    "loading": "جاري التحميل", "error": "وقع شي مشكل", "save": "حفظ",
    "cancel": "إلغاء", "confirm": "تأكيد", "close": "إغلاق", "back": "رجوع",
    "next": "التالي", "previous": "السابق", "search": "بحث", "filter": "تصفية",
    "sort": "ترتيب", "refresh": "تحديث", "export": "تصدير"
  },
  "nav": {
    "dashboard": "لوحة القيادة", "contacts": "العملاء", "companies": "الشركات",
    "deals": "الصفقات", "policies": "العقود", "claims": "الحوادث",
    "broker_queue": "لائحة الانتظار", "settings": "الإعدادات", "profile": "الملف الشخصي"
  },
  "auth": {
    "login": "دخول", "logout": "خروج", "signin": "تسجيل الدخول",
    "signup": "إنشاء حساب", "forgotPassword": "نسيت كلمة السر",
    "resetPassword": "إعادة تعيين كلمة السر", "verifyEmail": "تأكيد البريد",
    "verifyMfa": "تأكيد رمز MFA", "selectTenant": "اختار المكتب",
    "aria": { "back_home": "الرجوع للرئيسية" },
    "footer": {
      "copyright": "حقوق {year} Skalean InsurTech محفوظة.",
      "privacy": "سياسة الخصوصية", "cookies": "الكوكيز", "cndp_notice": "إشعار CNDP"
    }
  },
  "tenant": {
    "switcher": { "label": "مكتب السمسرة", "placeholder": "اختار مكتب" },
    "required": "خاصك تختار مكتب باش تكمل", "access_denied": "ما عندكش الإذن لهاد المكتب"
  },
  "errors": {
    "network": "كاينة مشكلة فـ الشبكة", "unauthorized": "خاصك تسجل الدخول",
    "forbidden": "ما عندكش الإذن", "notFound": "ما لقيناش هاد الحاجة",
    "serverError": "مشكل فـ السيرفر", "validation": "المعطيات غالط",
    "session_expired": "الجلسة ديالك سالات. عاود سجل الدخول.",
    "refresh_failed": "ما قدرناش نمددو الجلسة",
    "tenant_required": "خاصك تختار مكتب السمسرة باش تكمل"
  },
...
### 6.26 `repo/apps/web-broker/src/messages/ar.json` (~120 lignes arabe classique)

```json
{
  "meta": {
    "title": { "default": "سكاليان للوسطاء", "brand": "سكاليان للوسطاء" },
    "description": "منصة سكاليان لإدارة الوساطة في التأمين."
  },
  "home": {
    "title": "مرحباً بكم في منصة سكاليان للوسطاء",
    "palette": {
    ...
    "placeholder": { "banner": "ستُستبدل هذه الصفحة بلوحة قيادة الوسيط في النسخة 17." }
  },
  "common": {
    "loading": "جارٍ التحميل", "error": "حدث خطأ", "save": "حفظ",
    "cancel": "إلغاء", "confirm": "تأكيد", "close": "إغلاق", "back": "رجوع",
    "next": "التالي", "previous": "السابق", "search": "بحث", "filter": "تصفية",
    "sort": "ترتيب", "refresh": "تحديث", "export": "تصدير"
  },
  "nav": {
    "dashboard": "لوحة القيادة", "contacts": "جهات الاتصال", "companies": "الشركات",
    "deals": "الصفقات", "policies": "العقود", "claims": "المطالبات",
    "broker_queue": "قائمة الانتظار", "settings": "الإعدادات", "profile": "الملف الشخصي"
  },
  "auth": {
    "login": "تسجيل الدخول", "logout": "تسجيل الخروج", "signin": "الدخول",
    "signup": "إنشاء حساب", "forgotPassword": "نسيت كلمة المرور",
    "resetPassword": "إعادة تعيين كلمة المرور", "verifyEmail": "التحقق من البريد",
    "verifyMfa": "التحقق من رمز MFA", "selectTenant": "اختيار الوكالة",
    "aria": { "back_home": "العودة إلى الصفحة الرئيسية" },
    "footer": {
      "copyright": "حقوق {year} سكاليان محفوظة.",
      "privacy": "سياسة الخصوصية", "cookies": "ملفات تعريف الارتباط", "cndp_notice": "إشعار CNDP"
    }
  },
  "tenant": {
    "switcher": { "label": "وكالة الوساطة", "placeholder": "اختر وكالة" },
    "required": "يلزم اختيار وكالة للمتابعة", "access_denied": "تم رفض الوصول إلى هذه الوكالة"
  },
  "errors": {
    "network": "تعذّر الاتصال بالشبكة", "unauthorized": "يلزم التسجيل",
    "forbidden": "الوصول مرفوض", "notFound": "العنصر غير موجود",
    "serverError": "خطأ في الخادم", "validation": "بيانات غير صالحة",
  ...
  "protected": {
    ...
    "placeholder": {
      "sidebar_note": "ستتم إضافة التنقل في Sprint 16 4.3.3.",
      "topbar_note": "سيتم تسليم الشريط العلوي في Sprint 16 4.3.3."
    }
  }
}
```
...
export type BrokerRole = 'broker_admin' | 'broker_user' | 'broker_assistant';
export type SuperRole = 'super_admin' | 'tenant_admin';
export type AnyRole = BrokerRole | SuperRole | 'garage_admin' | 'garage_operator' | 'customer' | 'assure';
  ...
  return new QueryClient({
    defaultOptions: {
      ...
      mutations: {
        onError: (error: any) => {
          Sentry.captureException(error, { tags: { component: 'query-mutation' } });
          logger.error({ component: 'query-client', err: error?.message, status: error?.status }, 'mutation_error');
          const status = error?.status ?? 0;
...
import { SignJWT } from 'jose';
import {
  decodeJwtUnsafe, isJwtExpired, getJwtExpiryDate, getJwtRemainingSeconds,
  jwtHasPermission, jwtHasRole, type SkaleanJwtPayload,
} from '@/lib/jwt';
...
async function makeToken(payload: Partial<SkaleanJwtPayload>, ttlSeconds = 900): Promise<string> {
  ...
  return new SignJWT(full as any).setProtectedHeader({ alg: 'HS256' }).sign(SECRET);
}
...
describe('jwt helpers', () => {
  ...
  it('isJwtExpired applies margin', async () => {
    const token = await makeToken({}, 15);
    const decoded = decodeJwtUnsafe(token);
    expect(isJwtExpired(decoded, 30)).toBe(true);
    expect(isJwtExpired(decoded, 5)).toBe(false);
  });
  ...
  it('jwtHasPermission detects permissions correctly', async () => {
    const token = await makeToken({ permissions: ['crm.contacts.read', 'crm.deals.write'] });
    const decoded = decodeJwtUnsafe(token);
    expect(jwtHasPermission(decoded, 'crm.contacts.read')).toBe(true);
    expect(jwtHasPermission(decoded, 'crm.contacts.delete')).toBe(false);
    expect(jwtHasPermission(null, 'crm.contacts.read')).toBe(false);
...
### 7.2 Vitest -- `src/lib/__tests__/api-client.spec.ts` (15 tests, ~350 lignes)
...
describe('api-client interceptors', () => {
  ...
  beforeEach(() => {
    ...
    useSessionStore.setState({
      session: {
        userId: 'user-1', email: 'test@example.ma', displayName: 'Test User',
        role: 'broker_admin', permissions: [], tenantId: 'tenant-test-1',
        preferredLocale: 'fr', mfaEnabled: false, accessToken: 'fake-jwt',
        expiresAt: new Date(Date.now() + 900_000).toISOString(),
      },
    });
    Object.defineProperty(document.documentElement, 'lang', { value: 'fr', configurable: true });
  ...
  it('handles 401 + refresh fail -> redirect login', async () => {
    global.fetch = vi.fn().mockResolvedValueOnce({
      ok: false, status: 401, json: async () => ({ code: 'REFRESH_FAILED' }),
    } as Response) as Mock;
    const client = createApiClient();
    mock = new MockAdapter(client);
    mock.onGet('/api/v1/protected').reply(401, {});
  ...
  it('throws ApiError typed on 5xx', async () => {
    const client = createApiClient();
    mock = new MockAdapter(client);
    mock.onGet('/api/v1/test').reply(500, { code: 'INTERNAL', message: 'Server error' });
    try {
      await client.get('/api/v1/test');
      expect.fail('Should have thrown');
    } catch (err: any) {
  ...
  it('refresh promise singleton deduplicates parallel 401s', async () => {
    const fetchSpy = vi.fn().mockResolvedValue({
      ok: true, status: 200, json: async () => ({ success: true, expires_in: 900 }),
    } as Response);
    global.fetch = fetchSpy as Mock;
    const client = createApiClient();
    mock = new MockAdapter(client);
    mock.onGet('/api/v1/test1').replyOnce(401).onGet('/api/v1/test1').reply(200, {});
    mock.onGet('/api/v1/test2').replyOnce(401).onGet('/api/v1/test2').reply(200, {});
    await Promise.all([client.get('/api/v1/test1'), client.get('/api/v1/test2')]);
    expect(fetchSpy).toHaveBeenCalledTimes(1);
  });

  it('correlates trace-id across retry', async () => {
    const client = createApiClient();
    mock = new MockAdapter(client);
    const traceIds: string[] = [];
    mock.onGet('/api/v1/test').reply((config) => {
      traceIds.push(String(config.headers!['x-trace-id']));
      if (traceIds.length === 1) return [503, {}];
      return [200, {}];
    });
    await client.get('/api/v1/test');
...
import { redirect as redirectMock } from 'next/navigation';
import {
  getSession, requireSession, getCurrentTenantId,
  requireTenant, requireRole, requirePermission,
} from '@/lib/auth/session';
import { SignJWT } from 'jose';
...
describe('auth session helpers', () => {
  ...
  it('getSession returns UserSession when valid token', async () => {
    const token = await makeToken({
      sub: 'user-1', role: 'broker_admin', permissions: ['crm.contacts.read'],
      tenant_id: 'tenant-1', email: 'admin@cabinet.ma',
    });
    setupCookie({ access_token: token, current_tenant_id: 'tenant-1' });
    const session = await getSession();
    expect(session?.userId).toBe('user-1');
  ...
  it('requireRole redirects when role mismatches', async () => {
    const token = await makeToken({
      sub: 'user-1', role: 'broker_user', permissions: [], tenant_id: 'tenant-1',
    });
    setupCookie({ access_token: token, current_tenant_id: 'tenant-1' });
    await expect(requireRole('broker_admin')).rejects.toThrow('REDIRECT:/fr/dashboard?error=forbidden');
  });
  ...
  it('requirePermission allows when permission present', async () => {
    const token = await makeToken({
      sub: 'user-1', role: 'broker_admin',
      permissions: ['crm.contacts.delete'], tenant_id: 'tenant-1',
    });
    setupCookie({ access_token: token, current_tenant_id: 'tenant-1' });
    const session = await requirePermission('crm.contacts.delete');
    expect(session.userId).toBe('user-1');
...
import { describe, it, expect } from 'vitest';
import {
  SUPPORTED_LOCALES, DEFAULT_LOCALE, RTL_LOCALES,
  getDirection, isSupportedLocale,
  LOCALE_LABELS, LOCALE_NATIVE_LABELS,
} from '@/lib/i18n/config';
...
function buildRequest(pathname: string, cookies: Record<string, string> = {}, headers: Record<string, string> = {}): NextRequest {
  const url = new URL(`http://localhost:3001${pathname}`);
...
test.describe('middleware auth guard', () => {
  ...
  test('select-tenant accessible to authenticated user without tenant', async ({ page, context }) => {
    ...
    const payload = Buffer.from(JSON.stringify({
      sub: 'user-test', role: 'broker_admin', permissions: [],
      exp: nowSec + 900, iat: nowSec,
    })).toString('base64url').replace(/=+$/, '');
    const header = Buffer.from(JSON.stringify({ alg: 'none', typ: 'JWT' }))
      .toString('base64url').replace(/=+$/, '');
    const token = `${header}.${payload}.`;
    await context.addCookies([{
      name: 'access_token', value: token, domain: 'localhost', path: '/', httpOnly: false,
    }]);
    await page.goto('http://localhost:3001/fr/select-tenant');
    expect(page.url()).toContain('/fr/select-tenant');
  });
  ...
  test('authenticated user on /login redirects to /dashboard', async ({ page, context }) => {
    ...
    const payload = Buffer.from(JSON.stringify({
      sub: 'user-test', role: 'broker_admin', permissions: [],
      tenant_id: 'tenant-1', exp: nowSec + 900, iat: nowSec,
    })).toString('base64url').replace(/=+$/, '');
    const header = Buffer.from(JSON.stringify({ alg: 'none', typ: 'JWT' }))
      .toString('base64url').replace(/=+$/, '');
    const token = `${header}.${payload}.`;
...
test.describe('tenant cookie + locale persist', () => {
  ...
  test('current_tenant_id cookie readable by client JS (not httpOnly)', async ({ context, page }) => {
    await context.addCookies([{
      name: 'current_tenant_id', value: 'tenant-abc-123',
...
| Variable | Default | Required | Securite | Description |
|----------|---------|----------|----------|-------------|
| `NEXT_PUBLIC_API_URL` | `http://localhost:4000` | OUI | CLIENT-SAFE | URL backend NestJS -- prod `https://api.skalean-insurtech.ma` (Atlas Cloud Benguerir) |
| `NEXT_PUBLIC_APP_URL` | `http://localhost:3001` | OUI | CLIENT-SAFE | URL canonique app (metadataBase SEO) |
| `NEXT_PUBLIC_APP_NAME` | `skalean-broker` | OUI | CLIENT-SAFE | Identifiant app pour tags Sentry + logs |
| `NEXT_PUBLIC_APP_VERSION` | `0.1.0` | NON | CLIENT-SAFE | Version semver injectee dans health endpoint |
| `NEXT_PUBLIC_DEFAULT_LOCALE` | `fr` | OUI | CLIENT-SAFE | Locale par defaut |
| `NEXT_PUBLIC_SUPPORTED_LOCALES` | `fr,ar-MA,ar` | OUI | CLIENT-SAFE | Liste CSV locales supportees |
| `NEXT_PUBLIC_TENANT_ID_HEADER` | `x-tenant-id` | OUI | CLIENT-SAFE | Header HTTP propagation tenant (decision-002) |
| `NEXT_PUBLIC_TRACE_ID_HEADER` | `x-trace-id` | NON | CLIENT-SAFE | Header HTTP correlation tracing |
| `NEXT_PUBLIC_IDEMPOTENCY_HEADER` | `Idempotency-Key` | NON | CLIENT-SAFE | Header HTTP idempotence mutations |
| `NEXT_PUBLIC_AUTH_REFRESH_PATH` | `/api/auth/refresh` | OUI | CLIENT-SAFE | Path Next.js API route refresh |
| `NEXT_PUBLIC_AUTH_LOGIN_PATH` | `/login` | OUI | CLIENT-SAFE | Path page login |
| `NEXT_PUBLIC_AUTH_LOGOUT_PATH` | `/api/auth/signout` | OUI | CLIENT-SAFE | Path API route signout |
| `NEXT_PUBLIC_SESSION_TTL_SEC` | `900` | NON | CLIENT-SAFE | TTL access_token (15 min) |
| `NEXT_PUBLIC_REFRESH_TTL_SEC` | `2592000` | NON | CLIENT-SAFE | TTL refresh_token (30 jours) |
| `NEXT_PUBLIC_SENTRY_DSN` | (vide) | NON dev / OUI prod | CLIENT-SAFE | DSN Sentry public |
| `NEXT_PUBLIC_MAPBOX_TOKEN` | (vide) | NON | CLIENT-SAFE TOKEN PUBLIC RESTRICTED | Token Mapbox restraint domaine |
| `NEXT_PUBLIC_FEATURE_FLAGS_URL` | (vide) | NON | CLIENT-SAFE | URL service feature flags |
| `NEXT_PUBLIC_CDN_URL` | (vide) | NON prod / OUI | CLIENT-SAFE | URL CDN Atlas Cloud |
| `NEXT_PUBLIC_AI_GATEWAY_URL` | (vide) | NON | CLIENT-SAFE | URL AI Gateway Skalean |
| `NEXT_PUBLIC_GTM_ID` | (vide) | NON | CLIENT-SAFE | GTM ID |
| `NEXT_PUBLIC_DEBUG` | `false` | NON | CLIENT-SAFE | Active logs verbose |
| `NEXT_PUBLIC_LIGHTHOUSE_PROFILE` | `mobile` | NON | CLIENT-SAFE | Profil Lighthouse CI |
| `NEXT_PUBLIC_PWA_ENABLED` | `false` | NON | CLIENT-SAFE | Active PWA (faux pour web-broker) |
| `ATLAS_CLOUD_REGION` | `bgr` | OUI prod | PRIVATE | Region Atlas Cloud (Benguerir) |
| `ATLAS_CLOUD_STORAGE_BUCKET` | (vide) | OUI prod | PRIVATE | Bucket S3 Atlas Cloud |

Note securite : tout `NEXT_PUBLIC_*` est inline dans le bundle JS client. JAMAIS y mettre un secret.
...
pnpm --filter @insurtech/web-broker dev
curl -s http://localhost:3001/api/health | jq .
curl -s -L http://localhost:3001/fr -I | head -20
curl -s -L http://localhost:3001/fr/dashboard -I | head -20
curl -s -L -H "Accept-Language: ar-MA" http://localhost:3001/ -I | head -20
...
- 4 API routes Next.js (refresh, signout, me, switch-tenant)
- jose 5.9.6 + date-fns-tz 4.1.0 ajoutes
- 39 tests Vitest + Playwright
- 3 locales fr / ar-MA / ar enrichies"
```
...
- Commande : `pnpm --filter @insurtech/web-broker dev`
- Expected : log `ready - started server on 0.0.0.0:3001`, `curl http://localhost:3001/api/health` retourne 200
- Failure mode : port occupe -> `lsof -i :3001`, ou pnpm-lock conflit -> `pnpm install --force`

**V2 (P0) -- Middleware redirect non-auth vers /login**
- Commande : `curl -sI -L http://localhost:3001/fr/dashboard | grep -i location`
- Expected : `location: /fr/login?redirect=%2Ffr%2Fdashboard`
- Failure mode : matcher mal configure -> verifier `config.matcher` middleware.ts

**V3 (P0) -- Middleware locale detection + redirect URL**
- Commande : `curl -sI -L -H "Accept-Language: ar-MA,ar;q=0.9" http://localhost:3001/`
- Expected : redirect /ar-MA ou /ar
- Failure mode : verifier localePrefix: 'always' + localeDetection: true

**V4 (P0) -- Cookies tokens + tenant set after login**
- Commande : E2E test "authenticated user on /login redirects to /dashboard"
- Expected : access_token httpOnly + current_tenant_id non-httpOnly + redirect 307 /dashboard
- Failure mode : verifier headers Set-Cookie backend NestJS

**V5 (P0) -- x-tenant-id injecte dans requests API**
- Commande : test Vitest api-client.spec.ts "injects x-tenant-id"
- Expected : header present sur axios
- Failure mode : verifier TenantContextSync hydrate cookie

**V6 (P0) -- Providers wrappers fonctionnent**
- Commande : E2E navigation /fr verifier useQuery(['session'])
- Expected : QueryClient initialise, ReactQueryDevtools dev
- Failure mode : "useSession must be used within SessionProvider" -> verifier chain

**V7 (P0) -- Layouts (auth) et (protected) appliques**
- Commande : E2E visit /fr/login (auth) puis /fr/dashboard (protected)
- Expected : /login centered card, /dashboard avec sidebar + topbar placeholder
- Failure mode : route group parentheses mal places

**V8 (P0) -- API route /api/auth/refresh fonctionne**
- Commande : `curl -X POST http://localhost:3001/api/auth/refresh -b "refresh_token=fake"`
- Expected : 401 avec body { code: 'REFRESH_FAILED' }
- Failure mode : verifier export const runtime = 'nodejs'

**V9 (P0) -- API route /api/auth/signout clear cookies**
- Commande : `curl -X POST http://localhost:3001/api/auth/signout -b "access_token=fake" -I`
- Expected : Set-Cookie access_token=; Max-Age=0 + current_tenant_id=; Max-Age=0
- Failure mode : verifier clearAuthCookies() appel

**V10 (P0) -- 401 declenche refresh + retry transparent**
- Commande : test Vitest "handles 401 -> attempts refresh + retry"
- Expected : 1 call refresh + 1 retry + 200 final
- Failure mode : boucle infinie -> verifier originalConfig._retry flag

**V11 (P0) -- Idempotency-Key UNIQUEMENT mutations**
- Commande : test Vitest "injects Idempotency-Key on POST but NOT on GET"
- Expected : POST/PUT/PATCH/DELETE ont header, GET non
- Failure mode : cache poisoning -> verifier MUTATION_METHODS.has(method)

**V12 (P0) -- 3 locales fr / ar-MA / ar repondent 200**
- Commande : `curl -sI http://localhost:3001/fr && curl -sI http://localhost:3001/ar-MA && curl -sI http://localhost:3001/ar`
- Expected : 200 OK + Content-Language correspondant
- Failure mode : ar-MA non reconnu -> verifier SUPPORTED_LOCALES

**V13 (P0) -- dir=rtl sur ar/ar-MA, dir=ltr sur fr**
- Commande : `curl -s http://localhost:3001/ar | grep '<html'`
- Expected : `<html lang="ar" dir="rtl"`
- Failure mode : verifier getDirection() layout.tsx

**V14 (P0) -- Typecheck 0 erreur**
- Commande : `pnpm --filter @insurtech/web-broker typecheck`
- Expected : exit 0, Found 0 errors
- Failure mode : reparer types

**V15 (P0) -- Lint 0 warning**
- Commande : `pnpm --filter @insurtech/web-broker lint --max-warnings 0`
- Expected : exit 0
- Failure mode : resoudre ESLint warnings

### P1 (8 criteres importants)

**V16 (P1) -- Tests Vitest 100% pass min 25 tests**
- Commande : `pnpm --filter @insurtech/web-broker test`
- Expected : 25+ passed, coverage > 80%
- Failure mode : mocks zustand/next/headers manquants

**V17 (P1) -- Tests Playwright 100% pass min 14 tests**
- Commande : `pnpm --filter @insurtech/web-broker test:e2e`
- Expected : tous specs verts chromium
- Failure mode : webServer pas demarre

**V18 (P1) -- Refresh race condition deduplicated**
- Commande : test "refresh promise singleton deduplicates parallel 401s"
- Expected : UN SEUL call refresh meme avec 5 requetes 401 parallel
- Failure mode : verifier refreshPromise singleton

**V19 (P1) -- Header X-Frame-Options DENY**
- Commande : `curl -sI http://localhost:3001/fr | grep -i x-frame`
- Expected : X-Frame-Options: DENY
- Failure mode : verifier next.config.mjs headers

**V20 (P1) -- CSP header configure**
- Commande : `curl -sI http://localhost:3001/fr | grep -i content-security`
- Expected : Content-Security-Policy avec default-src 'self'
- Failure mode : CSP manquant ou permissif

**V21 (P1) -- Cookies httpOnly access_token**
- Commande : E2E context.cookies() check httpOnly: true sur access_token
- Expected : httpOnly true, secure prod, sameSite lax
- Failure mode : XSS leak risk

**V22 (P1) -- Sentry init prod uniquement**
- Commande : grep Sentry.init dans providers.tsx
- Expected : skip si NODE_ENV development OU dsn vide
- Failure mode : spam DSN dev

**V23 (P1) -- Switch tenant valide acces user**
- Commande : test Vitest switch-tenant
- Expected : 403 si pas membership
- Failure mode : securite cassee
...
- Commande : `pnpm --filter @insurtech/web-broker lh`
- Expected : categories.performance.score >= 0.70
- Failure mode : bundle gros -> optimizePackageImports

**V25 (P2) -- Lighthouse Accessibility >= 90**
- Expected : categories.accessibility.score >= 0.90
- Failure mode : aria manquants

**V26 (P2) -- Bundle JS first load < 250 ko gzipped**
- Commande : `pnpm --filter @insurtech/web-broker build` + analyse
- Expected : First Load JS < 250 ko
- Failure mode : tree-shake imports

**V27 (P2) -- Hydration 0 console.error sur boot**
- Commande : E2E `page.on('console', e => expect(e.type()).not.toBe('error'))`
- Expected : aucun message error
- Failure mode : hydration mismatch -> verifier conditionals SSR

**V28 (P2) -- Coverage Vitest >= 80% lib/**
- Commande : `pnpm --filter @insurtech/web-broker test --coverage`
- Expected : lib/api-client.ts, lib/auth/*.ts, lib/jwt.ts > 80%
- Failure mode : branches non testees
...
**EC1 -- Cookie access_token expire pendant navigation utilisateur**
Scenario : User a 30 sec restantes + clique sur /contacts. Pendant fetch RSC, JWT expire.
Solution : middleware Edge voit JWT expire -> redirect /login. Pour UX : api-client client-side detecte 401 + trigger refresh + reload.

**EC2 -- Refresh token race avec 5 tabs paralleles**
Scenario : User 5 tabs, tous voient access_token expire simultanement.
Solution : singleton `refreshPromise` deduplicate dans api-client.ts. Backend Sprint 5 deduplicate sur user_id (row lock).

**EC3 -- Cookie current_tenant_id corrompu (UUID invalide)**
Scenario : User edite DevTools, met `current_tenant_id=garbage`. Backend rejette 400.
Solution : api-client recoit 400 -> toast erreur + nettoie cookie + redirect /select-tenant.

**EC4 -- Multiple tabs avec tenants differents**
Scenario : Courtier multi-portefeuille ouvre 2 tabs avec 2 cabinets.
Solution : sessionStorage zustand isole par tab. Cookie shared (last write wins). Trade-off documente.

**EC5 -- Locale non supportee dans Accept-Language**
Scenario : Visiteur Allemand `de-DE,en;q=0.9`.
Solution : detectLocaleFromRequest fallback DEFAULT_LOCALE (fr).

**EC6 -- JWT malforme (corrompu ou tronque)**
Scenario : cookie access_token = "abc.def".
Solution : decodeJwtUnsafe try/catch retourne null -> middleware redirect /login + clear cookie.

**EC7 -- Backend API down pendant signout**
Scenario : User clique logout, backend timeout.
Solution : signout/route.ts best-effort -- meme si backend echoue, clear cookies cote client.

**EC8 -- Edge Runtime limit 1 MB bundle middleware**
Scenario : Trop de libs lourdes dans middleware.
Solution : middleware utilise UNIQUEMENT jose + next-intl middleware. Pas axios, pas date-fns.

**EC9 -- Cookie NEXT_LOCALE manquant ET Accept-Language vide**
Scenario : Curl sans headers.
Solution : fallback DEFAULT_LOCALE (fr).

**EC10 -- User authentifie mais tenant_id absent dans JWT**
Scenario : User vient de signup, pas encore selectionne cabinet.
Solution : middleware redirect /select-tenant. (auth)/layout.tsx accepte la route.

**EC11 -- Server Component apiServerGet timeout**
Scenario : Backend lent (3s+).
Solution : prefetchQuery soft-fails return null. UI fallback "Tenant indisponible". Logger error.

**EC12 -- next-intl locale switcher casse avec next/navigation native**
Scenario : Dev utilise useRouter next/navigation au lieu de createNavigation(routing).
Solution : createNavigation exporte typed helpers. Doc dans routing.ts.
...
1. **Cookies essentiels** : access_token, refresh_token, current_tenant_id, NEXT_LOCALE sont necessaires au fonctionnement. Sous CNDP, ces cookies essentiels ne necessitent pas consentement explicite mais doivent etre listes dans politique cookies (Sprint 16 tache 4.3.13 `/legal/cookies`).

2. **Cookies analytics + tracking** : Sentry, GTM, Mapbox necessitent consentement explicite (banner livre Sprint 17). Sprint 16 : Sentry charge UNIQUEMENT en prod + DSN configure.

3. **Droit d'effacement (Art. 7 loi 09-08)** : utilisateur peut demander purge. Endpoint `/api/auth/signout` clear cookies. Sprint 6 livre procedure CNDP purge backend.

4. **Notification breach 72h** : Sentry alerte Sprint 12 (compliance audit) + escalade CNDP automatique via Atlas Cloud SOC.

### Loi 53-05 (Echanges Electroniques)

- Chiffrement transit TLS 1.3 obligatoire (configure Sprint 4 headers).
- Cookies secure: true en production.
- sameSite: 'lax' anti-CSRF baseline.
...
- Duration max cookies non-essentiels : 13 mois.
- Banner cookie consent obligatoire sites publics (Sprint 17).
- Sites prives B2B (web-broker authentifie) exemptes mais doivent afficher politique cookies.

Cette tache fournit le lien footer `/legal/cookies` dans (auth)/layout.tsx (page livree Sprint 16 tache 4.3.13).

### Cloud souverain Atlas Cloud Benguerir (decision-008)

- NEXT_PUBLIC_API_URL prod = `https://api.skalean-insurtech.ma` (Atlas Cloud Benguerir DC1/DC2).
- NEXT_PUBLIC_CDN_URL prod = `https://cdn.skalean-insurtech.ma` (Atlas Cloud Object Storage).
- next.config.mjs images.remotePatterns exclut tout `*.amazonaws.com`.
- Variables env production injectees par secret manager Atlas Cloud (jamais en clair repo).
- Tier IV DC2 disponibilite 99.99% -- SLA web-broker base sur cette garantie.

### ACAPS

ACAPS deja client Atlas Cloud Services -- coherence ecosystem facilite audits Programme Emergence. Cette tache ne touche pas endpoints reglementaires (reporting ACAPS = Sprint 31).

---

## 13. Conventions absolues skalean-insurtech (20 conventions)

1. **Multi-tenant strict** : tout endpoint API recoit x-tenant-id header (decision-002). Automatise injection client (zustand) + server (cookie). AsyncLocalStorage backend (Sprint 6).

2. **Zod validation only** : aucune yup/joi. switch-tenant utilise `z.object({ tenant_id: z.string().uuid() }).parse(body)`.

3. **Pino logger (jamais console.log)** : console interdit production (decision-006 etendue). Utilise `logger.info/warn/error/debug` depuis `@/lib/logger`.

4. **Argon2id (jamais bcrypt)** : pas applicable cette tache. Convention rappelee : aucun bcrypt cote serveur.

5. **pnpm only** : pnpm-workspace.yaml + pnpm-lock.yaml exclusivement. Pas de npm/yarn.

6. **TypeScript strict** : tsconfig.json strict + noUncheckedIndexedAccess + noImplicitOverride. Aucun any non documente.

7. **RBAC defense profondeur** : UI cache features (Sprint 16 tache 4.3.12) MAIS backend toujours valide (Sprint 7 PermissionGuard).

8. **Kafka events** : pas applicable. Mention rappelee : signals critiques (auth.signin, tenant.switched) emis Kafka backend Sprint 5+.

9. **@insurtech/* imports** : aliases `@insurtech/shared-ui/*`, `@insurtech/shared-config/*`, `@insurtech/shared-types/*` (Sprint 4). Imports relatifs hors meme module interdits.

10. **NO EMOJI ABSOLU (decision-006)** : zero emoji code, JSON, commits, logs, tests. CI block PR si detecte.

11. **Idempotency-Key sur mutations** : POST/PUT/PATCH/DELETE seulement. JAMAIS GET (cache poisoning).

12. **Conventional Commits** : feat(sprint-16), fix(...), refactor(...). Metadata Task/Sprint/Phase/Refs.

13. **Cloud Atlas Maroc Benguerir (decision-008)** : api.skalean-insurtech.ma, cdn.skalean-insurtech.ma, s3.bgr.atlascloudservices.ma. Jamais AWS.

14. **Sofidemy brand kit** : #E95D2C (Orange), #1A2730 (Navy), #B0CEE2 (Sky Blue), #2D5773 (ACAPS Teal). Theme color viewport mobile.

15. **Fonts** : Montserrat (latin), Noto Naskh Arabic (arabe), Geist Mono (code). next/font/google display: 'swap', preload: true Montserrat + Noto.

16. **Timezone Africa/Casablanca** : tous dates formattees ce timezone. formats.ts `formatInTimeZone(date, 'Africa/Casablanca')`. NextIntlClientProvider timeZone="Africa/Casablanca".

17. **Currency MAD** : tous montants en MAD. formatCurrency utilise currency: 'MAD'. NextIntlClientProvider formats.number.currency.

18. **WCAG 2.1 AA** : contraste 4.5:1, focus visible, aria labels, keyboard navigation. aria-label sur boutons placeholders. Lighthouse Accessibility >= 90.

19. **Skalean AI frontier (decision-005)** : pas integration AI directe. NEXT_PUBLIC_AI_GATEWAY_URL reservee. Sprint 13+ ajoute useAiGateway().

20. **SetUp Sprint 4 base** : aucune deviation bootstrap Sprint 4. Etend uniquement.
...
- middleware.ts (compose i18n + auth + tenant)
- src/app/[locale]/layout.tsx (root layout enrichi)
- src/app/[locale]/(auth)/layout.tsx (centered card pages publiques)
- src/app/[locale]/(protected)/layout.tsx (guard auth + tenant + RSC prefetch)
- src/app/api/auth/{refresh,signout,me,switch-tenant}/route.ts
- src/app/api/health/route.ts
- src/components/providers.tsx (HydrationBoundary + Sentry)
...
- src/lib/api-client.ts (refresh singleton + Idempotency-Key + 401 retry)
- src/lib/api-client.server.ts (server-only fetch with cookies)
- src/lib/jwt.ts (jose decodeJwtUnsafe + helpers permissions)
- src/lib/auth/{session,cookies}.ts (Server Component helpers)
- src/lib/i18n/{config,request,routing,formats}.ts
- src/lib/query-client.ts (singleton + retry policy)
- src/store/{session-store,tenant-store}.ts (zustand stores)
- src/messages/{fr,ar-MA,ar}.json (etendus auth + tenant + middleware)
- src/types/{session,api}.d.ts
...
Tests (39 nouveaux) :
- 8 tests jwt helpers
- 15 tests api-client interceptors
- 6 tests auth session helpers (Server Components)
- 5 tests i18n config
- 12 tests middleware compose
- 4 tests E2E Playwright boot + locales
...
Validation V1-V28 : 15 P0 + 8 P1 + 5 P2 -- TOUS PASSED.

Co-authored-by: Saad Belghali Acharki
Co-authored-by: Abla Mounjia
```
...
- consomme `lib/i18n/formats.ts` pour `formatPhoneNumber` (signup)
- ajoute pages `(auth)/login/page.tsx`, `(auth)/verify-mfa/page.tsx`, `(auth)/signup/page.tsx`, `(auth)/forgot-password/page.tsx`, `(auth)/reset-password/page.tsx`, `(auth)/verify-email/page.tsx`, `(auth)/select-tenant/page.tsx`, `(auth)/email-sent/page.tsx`
- supprime placeholder.tsx livres ici
...
## 17. Annexes -- Patterns supplementaires production-ready

### 17.1 Pattern d'usage Server Component routes protegees (exemple Sprint 16 tache 4.3.4)
...
export default async function DashboardPage({ params, searchParams }: DashboardPageProps) {
  ...
  const widgets = await apiServerGet<DashboardWidgets>('/api/v1/analytics/dashboard', {
    searchParams: { date_start: sp.date_start, date_end: sp.date_end },
  });
...
### 17.2 Pattern d'usage Client Component pages auth (exemple Sprint 16 tache 4.3.2)
...
export default function LoginPage() {
  ...
  async function onSubmit(values: SignInForm) {
    setLoading(true);
    try {
      const response = await apiPost<SignInResponse, SignInForm>('/api/v1/auth/signin', values);
      if (response.needs_mfa && response.mfa_challenge_token) {
        sessionStorage.setItem('mfa_challenge_token', response.mfa_challenge_token);
        router.push('/verify-mfa');
        return;
  ...
  return <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4"></form>;
}
```

### 17.3 Pattern d'usage RBAC UI conditionnel (exemple Sprint 16 tache 4.3.12)
...
// Usage example dans Sidebar (Sprint 16 tache 4.3.3) :
// <HasRole role="broker_admin">
//   <SidebarItem href="/parametres" label="Parametres" />
// </HasRole>
```

### 17.4 Script `repo/scripts/validate-i18n-keys.ts` (~85 lignes)
...
console.log(`i18n validation OK -- ${app} : ${referenceKeys.size} keys parite verifiee.`);
```

### 17.5 Script `repo/scripts/check-no-emoji.sh`
...
echo "[check-no-emoji] OK -- aucun emoji detecte"
```

### 17.6 Pieges techniques additionnels (PT16-PT25)

**PT16 -- `redirect()` Next.js 15 throws (n'est pas return)** : `redirect('/login')` jette une `NEXT_REDIRECT` error interceptee par framework. Ne JAMAIS try/catch global Server Components qui appellent redirect.

**PT17 -- middleware Edge Runtime non-deterministe sans config** : sans config, Next.js peut cacher la response middleware. Mitigation : NextResponse.next({ headers }) avec headers variants doit invalider auto, mais verifier CI.

**PT18 -- `await cookies()` dans fonction sync** : Next.js 15 cookies() async. Si on oublie await -> retourne Promise<ReadonlyRequestCookies>. Mitigation : strict TS + ESLint rule no-floating-promises.

**PT19 -- Date.now() server VS client decales** : tests Vitest avec vi.setSystemTime impacte uniquement contexte test. Mitigation : toujours utiliser UTC dans tests (`new Date('2026-05-15T10:00:00Z')`).

**PT20 -- next-intl getMessages cote Server peut renvoyer mauvais cache** : Next.js 15 caching defaults no-store mais next-intl peut cacher messages. Mitigation : restart dev server apres edit messages.

**PT21 -- setRequestLocale obligatoire avant getTranslations** : sinon MISSING_LOCALE runtime. Convention : premiere ligne apres await params dans tout Server Component.

**PT22 -- HydrationBoundary deserialise Date objects en string** : Mitigation : normaliser ISO string cote serveur AVANT dehydrate.

**PT23 -- Sentry init double via React Strict Mode dev** : Strict Mode monte 2x effects. Mitigation : useRef flag sentryInitialized.current = true apres premier init.

**PT24 -- Cookie secure: true echoue dev http** : Mitigation : secure: isProd (false dev).

**PT25 -- dehydrate(queryClient) ne serialise pas Error objects** : Mitigation : pour queries qui peuvent fail (apiServerGet), wrap try/catch + return null.

### 17.7 Edge cases supplementaires (EC13-EC20)

**EC13 -- IPv6 cookies par defaut domain** : si user accede `http://[::1]:3001` au lieu de localhost. Mitigation : middleware accepte les deux + cookie domain auto-detect.

**EC14 -- Cookie size limit 4 KB** : JWT compact peut atteindre 1-2 KB. Mitigation : JWT minimaliste (sub + role + permissions court + exp). Permissions complete via /auth/me.

**EC15 -- Backend retourne tenant_id different de cookie** : edge case desync. Mitigation : api-client privilegie cookie/zustand. Backend valide x-tenant-id contre membership.

**EC16 -- Refresh token revoked mid-session** : admin revoke session backend. Prochain refresh -> 401. Mitigation : clear cookies + redirect login + toast "Session revoquee".

**EC17 -- User change role pendant session** : admin retire role. Cache permissions client obsolete. Mitigation : polling /auth/me every 5min sync permissions. Backend rejette 403.

**EC18 -- Multi-tab login simultane** : 2 onglets /login. Premier succes set cookies, second succes set autres cookies. Mitigation : backend Sprint 5 deduplicate ou refresh_token rotation invalide first.

**EC19 -- Cookie SameSite=strict avec OAuth callback** : OAuth callback redirect 3rd party perdrait cookies strict. Mitigation : sameSite: 'lax' accepte (access_token + tenant_id).

**EC20 -- DNS resolve Atlas Cloud failure prod** : api.skalean-insurtech.ma DNS unavailable. Mitigation : Cloudflare CDN cache + health check Atlas SOC alerte + DC2 failover.

### 17.8 Mapping endpoints API consommes

| Endpoint backend NestJS | Methode | Consume par | Notes |
|--------------------------|---------|--------------|-------|
| `/api/v1/auth/refresh` | POST | `/api/auth/refresh` proxy | Sprint 5 -- rotation refresh_token |
| `/api/v1/auth/signout` | POST | `/api/auth/signout` proxy | Sprint 5 -- revoke session + audit |
| `/api/v1/auth/me` | GET | `/api/auth/me` proxy | Sprint 5 + Sprint 7 |
| `/api/v1/tenants/me` | GET | `/api/auth/switch-tenant` validation | Sprint 6 -- memberships |
| `/api/v1/auth/signin` | POST | Sprint 16 tache 4.3.2 (api-client direct) | NOT consume cette tache |
| `/api/v1/auth/signup` | POST | Sprint 16 tache 4.3.2 | NOT consume cette tache |
| `/api/v1/auth/verify-mfa` | POST | Sprint 16 tache 4.3.2 | NOT consume cette tache |

### 17.9 Performance attendue
...
### 17.10 Verification finale "Definition of Done"
