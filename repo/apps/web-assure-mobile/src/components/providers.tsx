...
/**
 * Client-side providers -- web-assure-mobile
 ...
 *   - Assure tenant context sync (cookie -> zustand)
 */
...
export function Providers({ children, locale }: ProvidersProps) {
  ...
  useEffect(() => {
    ...
    const cookieMatch = document.cookie.match(/(?:^|;\s*)skalean\.assure_tenant_id=([^;]+)/);
    if (cookieMatch?.[1]) {
      useAssureTenantStore.getState().setTenantId(decodeURIComponent(cookieMatch[1]));
    }
  }, []);
