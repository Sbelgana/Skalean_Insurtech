/**
 * api-client.spec.ts -- web-assure-mobile
...
describe('api-client', () => {
  ...
  it('injects x-tenant-id from zustand store', async () => {
    ...
    await client.get('/api/v1/claims');
  ...
  it('captures 5xx errors in Sentry', async () => {
    ...
    await expect(client.get('/api/v1/claims')).rejects.toThrow();
    expect(Sentry.captureException).toHaveBeenCalledTimes(1);
  });

  it('does NOT capture 4xx errors in Sentry', async () => {
    mock.onGet('/api/v1/claims/missing').reply(404);
    await expect(client.get('/api/v1/claims/missing')).rejects.toThrow();
