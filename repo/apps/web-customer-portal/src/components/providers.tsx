...
/**
 * Client-side providers -- web-customer-portal
...
export function Providers({ children, locale }: ProvidersProps) {
  ...
  useEffect(() => {
    ...
    const cookieMatch = document.cookie.match(/(?:^|;\s*)skalean\.customer_tenant_id=([^;]+)/);
    if (cookieMatch?.[1]) {
      useCustomerTenantStore.getState().setTenantId(decodeURIComponent(cookieMatch[1]));
    }
  }, []);
