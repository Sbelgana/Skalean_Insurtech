...
/**
 * Client-side providers -- web-garage-mobile
 ...
 *   - Garage tenant context sync (cookie -> zustand)
 */
...
export function Providers({ children, locale }: ProvidersProps) {
  ...
  useEffect(() => {
    ...
    const cookieMatch = document.cookie.match(/(?:^|;\s*)skalean\.garage_id=([^;]+)/);
    if (cookieMatch?.[1]) {
      useGarageTenantStore.getState().setTenantId(decodeURIComponent(cookieMatch[1]));
    }
  }, []);
