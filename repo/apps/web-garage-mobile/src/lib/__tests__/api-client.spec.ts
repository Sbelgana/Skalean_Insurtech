/**
 * api-client.spec.ts -- web-garage-mobile
...
import { useGarageTenantStore } from '@/store/garage-tenant-store';
...
describe('api-client', () => {
  ...
  it('injects x-tenant-id from zustand store', async () => {
    useGarageTenantStore.getState().setTenantId('garage-abc-123');
    let capturedHeaders: Record<string, unknown> = {};
    mock.onGet('/api/v1/work-orders').reply((config) => {
      capturedHeaders = config.headers as Record<string, unknown>;
      return [200, []];
    });
    await client.get('/api/v1/work-orders');
    expect(capturedHeaders['x-tenant-id']).toBe('garage-abc-123');
  });
  ...
  it('injects Idempotency-Key on POST mutations', async () => {
    let capturedKey = '';
    mock.onPost('/api/v1/work-orders').reply((config) => {
      capturedKey = (config.headers as Record<string, string>)['Idempotency-Key'] ?? '';
      return [201, { id: '1' }];
    });
    await client.post('/api/v1/work-orders', { name: 'Reparation carrosserie' });
    expect(capturedKey).toBeTruthy();
  });

  it('injects Idempotency-Key on PUT/PATCH/DELETE', async () => {
    let putKey = '', patchKey = '', deleteKey = '';
    mock.onPut('/api/v1/work-orders/1').reply((c) => { putKey = (c.headers as Record<string, string>)['Idempotency-Key'] ?? ''; return [200, {}]; });
    mock.onPatch('/api/v1/work-orders/1').reply((c) => { patchKey = (c.headers as Record<string, string>)['Idempotency-Key'] ?? ''; return [200, {}]; });
    mock.onDelete('/api/v1/work-orders/1').reply((c) => { deleteKey = (c.headers as Record<string, string>)['Idempotency-Key'] ?? ''; return [204]; });
    await client.put('/api/v1/work-orders/1', {});
    await client.patch('/api/v1/work-orders/1', {});
    await client.delete('/api/v1/work-orders/1');
  ...
  it('captures 5xx errors in Sentry', async () => {
    mock.onGet('/api/v1/work-orders').reply(503);
    await expect(client.get('/api/v1/work-orders')).rejects.toThrow();
    expect(Sentry.captureException).toHaveBeenCalledTimes(1);
  });

  it('does NOT capture 4xx errors in Sentry', async () => {
    mock.onGet('/api/v1/work-orders/missing').reply(404);
    await expect(client.get('/api/v1/work-orders/missing')).rejects.toThrow();
