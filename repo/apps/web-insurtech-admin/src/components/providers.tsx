...
/**
 * Client-side providers -- web-insurtech-admin
 ...
 *   - Admin tenant context sync (cookie -> zustand)
 */
...
export function Providers({ children, locale }: ProvidersProps) {
  ...
  useEffect(() => {
    ...
    const cookieMatch = document.cookie.match(/(?:^|;\s*)skalean\.admin_tenant_id=([^;]+)/);
    if (cookieMatch?.[1]) {
      useAdminTenantStore.getState().setTenantId(decodeURIComponent(cookieMatch[1]));
    }
  }, []);
