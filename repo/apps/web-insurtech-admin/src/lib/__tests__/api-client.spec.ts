/**
 * api-client.spec.ts -- web-insurtech-admin
...
import { useAdminTenantStore } from '@/store/admin-tenant-store';
...
describe('api-client', () => {
  ...
  beforeEach(() => {
    ...
    useAdminTenantStore.getState().clearTenant();
  ...
  it('injects x-tenant-id from zustand store', async () => {
    useAdminTenantStore.getState().setTenantId('tenant-abc-123');
    let capturedHeaders: Record<string, unknown> = {};
    mock.onGet('/api/v1/tenants').reply((config) => {
  ...
  it('injects Idempotency-Key on POST mutations', async () => {
    ...
    await client.post('/api/v1/tenants', { name: 'Acme' });
    expect(capturedKey).toBeTruthy();
  });

  it('injects Idempotency-Key on PUT/PATCH/DELETE', async () => {
    let putKey = '', patchKey = '', deleteKey = '';
    mock.onPut('/api/v1/tenants/1').reply((c) => { putKey = (c.headers as Record<string, string>)['Idempotency-Key'] ?? ''; return [200, {}]; });
    mock.onPatch('/api/v1/tenants/1').reply((c) => { patchKey = (c.headers as Record<string, string>)['Idempotency-Key'] ?? ''; return [200, {}]; });
    mock.onDelete('/api/v1/tenants/1').reply((c) => { deleteKey = (c.headers as Record<string, string>)['Idempotency-Key'] ?? ''; return [204]; });
    await client.put('/api/v1/tenants/1', {});
    await client.patch('/api/v1/tenants/1', {});
    await client.delete('/api/v1/tenants/1');
  ...
  it('captures 5xx errors in Sentry', async () => {
    mock.onGet('/api/v1/tenants').reply(503);
    await expect(client.get('/api/v1/tenants')).rejects.toThrow();
    expect(Sentry.captureException).toHaveBeenCalledTimes(1);
  });

  it('does NOT capture 4xx errors in Sentry', async () => {
    mock.onGet('/api/v1/tenants/missing').reply(404);
    await expect(client.get('/api/v1/tenants/missing')).rejects.toThrow();
